 ...
 *
 */
public class LoopSolv {//used by puzzlegenv2 to verify the solution of the puzzle
	...
	public boolean generatorLeft(Integer[][] map, int currentX, int currentY) {
		////////////////////
		//System.out.print("\r");
		...
		if(map[currentX][currentY]!=0)//this part is used for solver (if this spot has a number in it, just jump to next one)
		{
			if (currentY < 8) {//moving on
				return generatorLeft(map, currentX, currentY+1);//the success or failure of built map will return to this point from last round
			} else {
				return generatorLeft(map, currentX+1, 0);//the success or failure of built map will return to this point from last round
			}
		}
		...
		while (!ifAcceptable) {//if the map is not successfully generated (last round failed), either will continue trying in this round or return the failure to the previous round level(when run out of number)
			...
			if (currentLeft.size() != 0) {
				map[currentX][currentY] = currentLeft.get(0);//if there are suitable numbers random select one to fill in this spot
				numberUsed.add(map[currentX][currentY]);//record the used number in this round
				if (currentY < 8) {//moving on
					ifAcceptable=generatorLeft(map, currentX, currentY+1);//the success or failure of built map will return to this point from last round
				} else {
					ifAcceptable=generatorLeft(map, currentX+1, 0);//the success or failure of built map will return to this point from last round
				}
	...
	public boolean generatorRight(Integer[][] map, int currentX, int currentY) {
		////////////////////
		//System.out.print("\r");
		//System.out.print(currentX+"||"+currentY);
		...
		{
			if (currentY < 8) {//moving on
				return generatorRight(map, currentX, currentY+1);//the success or failure of built map will return to this point from last round
			} else {
				return generatorRight(map, currentX+1, 0);//the success or failure of built map will return to this point from last round
			}
		}
		...
		while (!ifAcceptable) {//if the map is not successfully generated (last round failed), either will continue trying in this round or return the failure to the previous round level(when run out of number)
			...
			if (currentLeft.size() != 0) {
				map[currentX][currentY] = currentLeft.get(currentLeft.size()-1);//if there are suitable numbers random select one to fill in this spot
				numberUsed.add(map[currentX][currentY]);//record the used number in this round
				if (currentY < 8) {//moving on
					ifAcceptable=generatorRight(map, currentX, currentY+1);//the success or failure of built map will return to this point from last round
				} else {
					ifAcceptable=generatorRight(map, currentX+1, 0);//the success or failure of built map will return to this point from last round
				}
